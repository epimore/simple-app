...
export const conf = {
    ...
    // url: 'https://epimore.cn/live/521555_1727417550056.flv', // 视频源
    // poster: "/assets/video_poster.gif", // 视频封面
    autoplay: true, // 是否自动播放，不自动播放，浏览器有限制规则
    autoplayMuted: false, // 是否自动播放（静音播放）
    // isLive: true,
    plugins: [FlvPlugin],
    retryCount: 0, // 重试次数
    crossOrigin: true, // 是否允许跨域
    videoInit: true, // 是否默认初始化video，默认初始化，默认true
    playsinline: false, // 是否启用内联播放模式，仅移动端生效
    ...
    marginControls: true, // 是否开启画面和控制栏分离模式，不开启空间多一些
    ...
    playbackRate: [0.5, 1, 1.5, 2, 4], //传入倍速可选数组
    // enablePlayRate: false, //false，禁用倍速播放（即控制栏不显示）
